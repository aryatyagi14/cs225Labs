                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    //compare the heads of the 2 queues
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
    //put all leaf nodes into the singleQueue
    for (size_t i = 0; i < frequencies.size(); i++) {
        singleQueue.push(new TreeNode(frequencies[i]));
    }
    // repeat until there is only one node on both
    while (singleQueue.size() + mergeQueue.size() > 1) {
        ...
        TreeNode* merged = new TreeNode(tempL -> freq.getFrequency() + tempR -> freq.getFrequency());
...
//single value in singleQueue only
}
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        if (current -> right == NULL && current -> left == NULL) {
            //how to get char from string of 1s and 0s? 
