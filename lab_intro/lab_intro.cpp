...
//the multiple that the luminance should change by
				double multiple = 1.0 - (.005 * distance);
				pixel.l = multiple * (pixel.l);
			}
...
PNG illinify(PNG image) {
	for (unsigned int x = 0; x < image.width(); x++) {
		for (unsigned int y = 0; y < image.height(); y++) {
//get the pixel at this part
			HSLAPixel & pixel = image.getPixel(x,y);

//if the hue color is closer to orange 
...
//get the pixel from the second image
			HSLAPixel &si = secondImage.getPixel(x,y);
			HSLAPixel &fi = firstImage.getPixel(x,y);
//check luminance for second image pixel
			if (si.l == 1) {
				fi.l = fi.l + .2;
				if (fi.l > 1) {
					fi.l = 1;
