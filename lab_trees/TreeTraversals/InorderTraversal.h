...
class InorderTraversal : public TreeTraversal<T> {
  public: 
    ...
    void add(typename BinaryTree<T>::Node*& treeNode) {
        //adds the right sub tree and its children
        //before this, the stack justt has the very left elements
        ...
        /*
        std::stack<typename BinaryTree<T>::Node*> s;
        while (!stack.empty())
        {
          ...
          if (curr -> right != NULL) {
            ...
            typename BinaryTree<T>::Node* goleft = curr -> right;
            while(goleft -> left != NULL) {
              stack.push(goleft -> left);
              goleft = goleft -> left;
            }
            goleft = curr -> right;
            while(goleft -> left != NULL) {
